...
type LogClient interface {
	...
	ProduseStream(ctx context.Context, opts ...grpc.CallOption) (Log_ProduseStreamClient, error)
}
...
func (c *logClient) ProduseStream(ctx context.Context, opts ...grpc.CallOption) (Log_ProduseStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Log_ServiceDesc.Streams[1], "/log.v1.Log/ProduseStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &logProduseStreamClient{stream}
	return x, nil
}

type Log_ProduseStreamClient interface {
	Send(*ProduceRequest) error
	Recv() (*ProduceResponse, error)
	grpc.ClientStream
}

type logProduseStreamClient struct {
	grpc.ClientStream
}

func (x *logProduseStreamClient) Send(m *ProduceRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *logProduseStreamClient) Recv() (*ProduceResponse, error) {
...
type LogServer interface {
	...
	ProduseStream(Log_ProduseStreamServer) error
...
func (UnimplementedLogServer) ProduseStream(Log_ProduseStreamServer) error {
	return status.Errorf(codes.Unimplemented, "method ProduseStream not implemented")
}
...
func _Log_ProduseStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(LogServer).ProduseStream(&logProduseStreamServer{stream})
}

type Log_ProduseStreamServer interface {
	Send(*ProduceResponse) error
	Recv() (*ProduceRequest, error)
	grpc.ServerStream
}

type logProduseStreamServer struct {
	grpc.ServerStream
}

func (x *logProduseStreamServer) Send(m *ProduceResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *logProduseStreamServer) Recv() (*ProduceRequest, error) {
...
var Log_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ProduseStream",
			Handler:       _Log_ProduseStream_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
